...
export function plan(result) {
  ...
  for (const check of result.failed) {
    if (SETTINGS[check]) out.settings.push({ check, action: SETTINGS[check] });
      ...
      // the deletion ships in the repo's governance/harness-sync PR. The
      // scheduled workflow is retired (#355); the apply is a manual step.
      out.human.push(`${check}: delivered by the harness sync retraction PR — if none is open, run node tools/repos/sync-codex.mjs --apply --repo <name> with an explicit chores-dumb[bot] GH_TOKEN, then approve via npm run codex:approve`);
